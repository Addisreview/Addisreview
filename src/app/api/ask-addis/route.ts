...
export async function POST(request: NextRequest) {
  try {
    ...
    // 1. Query Supabase for relevant businesses using the existing search RPC
    const supabase = createClient(
      process.env.NEXT_PUBLIC_DB_HOST!,
      process.env.NEXT_PUBLIC_DB_ANON!
    );

    const { data: businesses } = await (supabase as any).rpc('search_businesses', {
    ...
    // Also pull top-rated as fallback if search returns nothing
    let pool = businesses || [];
    if (pool.length < 5) {
    ...
    // Deduplicate by id and cap at 40
...
If the user asks something unrelated to finding businesses in Ethiopia, politely redirect them. Never make up businesses not in the list. Always respond in English unless the user writes in Amharic, in which case respond in Amharic.`,
    ...
    console.error('ask-addis error:', err);
